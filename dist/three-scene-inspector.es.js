...
import { RepeatWrapping as b, ClampToEdgeWrapping as g, MirroredRepeatWrapping as m, NoColorSpace as f, SRGBColorSpace as v, LinearSRGBColorSpace as y, FrontSide as k, BackSide as x, DoubleSide as w, BoxHelper as S, CameraHelper as h, DirectionalLightHelper as C } from "three";
...
class O {
  constructor(e, t) {
    this.onSelectionChange = t, this.buttons = /* @__PURE__ */ new Map(), this.selectedObject = null, this.currentScene = null, this.folderObjects = /* @__PURE__ */ new Set(), this.excludeLabels = ["__inspector_bounding_box__"], this.pane = new u({ container: e, title: "Scene Tree" }), this.searchElement = this.createSearchElement();
  }
  get searchQuery() {
  ...
  createSearchElement() {
    ...
    return e.type = "text", e.placeholder = "Search...", e.className = "scene-tree-search", e.addEventListener("input", () => {
      this.currentScene && this.rebuildTree();
    }), this.pane.element.querySelector(".tp-rotv_b").insertAdjacentElement("afterend", e), e;
  }
  refresh(e) {
  ...
  dispose() {
    this.searchElement.remove(), this.pane.dispose();
  }
  excludeFromTree(...e) {
  ...
  buildHierarchy(e, t) {
    ...
    const n = this.getChildrenArray(e), r = this.searchQuery ? n.filter((a) => !this.shouldSkipObject(a) && this.objectOrDescendantMatches(a)) : n, i = this.createLabel(e);
    if (r.length > 0) {
      const a = this.searchQuery ? !0 : e.type !== "Bone";
      this.createFolderNode(e, t, i, r, a);
    } else
  ...
  shouldSkipObject(e) {
    ...
    return this.excludeLabels.some((n) => t.includes(n));
  }
  getChildrenArray(e) {
    const t = e.children;
    return t ? [...t] : [];
  }
  createFolderNode(e, t, n, r, i) {
    const a = t.addFolder({ title: n, expanded: i }), o = a.element.querySelector(".tp-fldv_b"), s = a.element.querySelector(".tp-fldv_m");
    o?.addEventListener("click", (l) => {
      this.isArrowClick(l.target, s) || (l.stopPropagation(), l.preventDefault(), this.selectObject(e));
    }, !0), this.buttons.set(e, { title: n, element: a.element }), this.folderObjects.add(e);
    for (const l of r)
  ...
  createLeafNode(e, t, n) {
    const i = ("isLight" in e && e.isLight ? this.lightsFolder : t).addButton({ title: n });
    i.on("click", () => this.selectObject(e)), this.buttons.set(e, i);
  }
  createLabel(e) {
    const t = e.constructor.name.replace(/_/g, ""), n = e.name || "";
    return n ? `${n} (${t})` : t;
  }
  updateSelectionIndicator() {
    this.buttons.forEach((e, t) => {
      const r = this.folderObjects.has(t) ? e.element.querySelector(".tp-fldv_b") : this.findLeafButtonElement(e.element), i = this.selectedObject === t;
  ...
  Texture: [
    ...
    { path: "material.map.repeat", type: "point" },
    {
      ...
      options: [
        ...
        { text: "ClampToEdgeWrapping", value: g },
...
class E {
  ...
  bind(e, t, n) {
    if (this.propertyExists(t, n.path))
      switch (n.type) {
        case "number":
          this.bindNumber(e, t, n);
          break;
        case "string":
          this.bindString(e, t, n);
          break;
        case "boolean":
          this.bindBoolean(e, t, n);
          break;
        case "point":
          this.bindPoint(e, t, n);
          break;
        case "color":
          this.bindColor(e, t, n);
          break;
        case "list":
          this.bindList(e, t, n);
          break;
        case "readonly":
          this.bindReadonly(e, t, n);
          break;
        case "image":
          this.bingImage(e, t, n);
          break;
        default:
          this.bindString(e, t, n);
          break;
  ...
  bindNumber(e, t, n) {
    const { target: r, key: i } = this.getTargetAndKey(t, n.path), a = this.buildBaseOptions(n);
    n.min !== void 0 && n.max !== void 0 && (a.min = n.min, a.max = n.max, a.step = n.step !== void 0 ? n.step : void 0), this.addBindingWithCallback(e, r, i, a);
  }
  bindString(e, t, n) {
    const { target: r, key: i } = this.getTargetAndKey(t, n.path), a = this.buildBaseOptions(n);
  ...
  bindPoint(e, t, n) {
    const { target: r, key: i } = this.getTargetAndKey(t, n.path), a = n.min !== void 0 ? n.min : -2e3, o = n.max !== void 0 ? n.max : 2e3, s = n.step !== void 0 ? n.step : void 0, l = {
      ...this.buildBaseOptions(n),
  ...
  bindColor(e, t, n) {
    const { target: r, key: i } = this.getTargetAndKey(t, n.path), a = {
      ...this.buildBaseOptions(n),
      view: "color"
    };
  ...
  bindList(e, t, n) {
    ...
    if (n.options && n.options.length > 0) {
      const s = typeof n.options[0] == "string" ? n.options.map((l) => ({ text: l, value: l })) : n.options;
  ...
  bindReadonly(e, t, n) {
    const r = n.path.split(".").pop() || n.path;
    let i = this.getValueFromPath(t, n.path);
    n.path === "name" && !i && (i = "(unnamed)");
    const a = {
      ...this.buildBaseOptions(n),
      readonly: !0
    };
  ...
  bingImage(e, t, n) {
    const { target: r, key: i } = this.getTargetAndKey(t, n.path);
  ...
  getTargetAndKey(e, t) {
    const n = t.split("."), r = this.getValueFromPath(e, t, !0), i = n[n.length - 1];
    return { target: r, key: i };
  }
  getValueFromPath(e, t, n = !1) {
    const r = t.split("."), i = n ? r.length - 1 : r.length;
    let a = e;
    for (let o = 0; o < i; o++)
      a = a?.[r[o]];
    return a;
  ...
  addBindingWithCallback(e, t, n, r) {
    const i = e.addBinding(t, n, r);
    return i.on("change", (a) => {
      this.onChange(n), r.needsUpdate && (t.needsUpdate = !0);
    }), i;
  }
  propertyExists(e, t) {
    const n = t.split(".");
    ...
    for (let i = 0; i < n.length - 1; i++)
      if (r = r?.[n[i]], r == null) return !1;
    return r != null && n[n.length - 1] in r;
  }
}
class P {
  constructor(e) {
  ...
  addHelperToggles(e, t) {
    this.helperDescriptors(t).forEach(({ label: n, createFn: r }) => {
      this.addHelperToggle(e, n, t, r);
    });
  }
  update() {
  ...
  helperDescriptors(e) {
    switch (e.type) {
      case "DirectionalLight": {
        const t = e, n = [
          { label: "Light Helper", createFn: () => new C(t, 1) }
        ];
        return t.castShadow && n.push({ label: "Shadow Camera", createFn: () => new h(t.shadow.camera) }), n;
      }
  ...
  enable(e, t, n) {
    const r = this.helperKey(e, t), i = n();
    this.scene.add(i), this.helpers.set(r, i);
  }
  disable(e, t) {
    const n = this.helperKey(e, t), r = this.helpers.get(n);
    r && (this.scene.remove(r), r.dispose(), this.helpers.delete(n));
  }
  addHelperToggle(e, t, n, r) {
    const i = this.helperKey(n, t), a = { show: this.helpers.has(i) };
    e.addBinding(a, "show", { label: t }).on("change", (s) => {
      s.value ? this.enable(n, t, r) : this.disable(n, t);
    });
  }
}
class F {
  constructor(e, t) {
    this.folders = /* @__PURE__ */ new Map(), this.selectedObject = null, this.propertiesTriggeringRefresh = [], this.propertiesTriggeringRebuild = ["castShadow"], this.isRefreshing = !1, this.pane = new u({ container: e, title: "Properties" }), this.pane.registerPlugin(B), this.helperManager = new P(t), this.propertyBinder = new E((n) => {
      this.isRefreshing || (this.propertiesTriggeringRefresh.includes(n) && this.pane.refresh(), this.propertiesTriggeringRebuild.includes(n) && setTimeout(() => this.refresh(), 0));
    });
  }
  setSelectedObject(e) {
  ...
  addProperies(e) {
    for (const [t, n] of Object.entries(T)) {
      const r = this.pane.addFolder({ title: t, expanded: !0 });
      for (const i of n)
        this.propertyBinder.bind(r, e, i);
      r.children.length === 0 ? r.dispose() : this.folders.set(t, r);
    }
  }
  addHelpersFolder(e) {
...
function L() {
  ...
  c.textContent = _, document.head.appendChild(c), d = !0;
}
class M {
  constructor(e) {
    this.panelsContainer = null, L(), this.scene = e;
    const { hierarchyContainer: t, propertyContainer: n } = this.createPanelContainers();
    this.hierarchyPanel = new O(t, this.onObjectSelected.bind(this)), this.propertyPanel = new F(n, this.scene), this.refresh();
  }
  createPanelContainers() {
    ...
    return n.id = "property-panel", e.appendChild(n), { hierarchyContainer: t, propertyContainer: n };
  }
  refresh() {
